...
function cleanQuery(value: string) {
  return value.replace(/[%_]/g, '').replace(/\s+/g, ' ').trim().slice(0, MAX_QUERY_LENGTH);
}
...
export async function GET(request: NextRequest) {
  ...
  const [peopleResponse, forumResponse, resourcesResponse, scholarshipsResponse] = await Promise.all([
    supabase
      .from('profiles')
      .select('id, full_name, department, current_level, executive_office')
      .or(`full_name.ilike.${pattern},department.ilike.${pattern},executive_office.ilike.${pattern}`)
  ...
  const errors = [peopleResponse.error, forumResponse.error, resourcesResponse.error, scholarshipsResponse.error].filter(Boolean);
  if (errors.length) {
    console.error('Search query error:', errors);
  ...
  const results: SearchResult[] = [
    ...(peopleResponse.data ?? []).map((row) => ({
      ...
      description: [row.executive_office, row.department, row.current_level].filter(Boolean).join(' • ') || 'DLCF AFIT member',
    ...
    ...(forumResponse.data ?? []).map((row) => ({
      id: row.id,
      title: row.title,
      description: row.content.length > 160 ? `${row.content.slice(0, 157)}...` : row.content,
      category: 'Forum' as const,
      href: '/fellowship/forum',
      meta: row.target_unit,
    })),
    ...(resourcesResponse.data ?? []).map((row) => ({
      ...
      meta: row.level ? `${row.level}` : undefined,
    })),
    ...(scholarshipsResponse.data ?? []).map((row) => ({
      ...
      meta: row.deadline ? new Date(row.deadline).toLocaleDateString() : undefined,
    })),
  ];
